...
class LogisticRegressionModel:
    ...
    def __init__(self, learning_rate=0.01, epochs=1000):
        ...
        #            Elbow Flare Score, Shoulder Asymmetry, Wrist Deviation]
        self.weights = [0.0] * 9
        self.trained = False
    ...
    def predict(self, knee_variance, knee_mean, stride_asymmetry, posture_score, fatigue_index,
                elbow_flare_score=0.0, shoulder_asymmetry=0.0, wrist_deviation=0.0):
        """
        Estimate injury probability and risk level.
        Accepts 8 biomechanical features (last 3 optional for backward compat).
        """
        if not self.trained:
            self.train_synthetic()

        # Normalize features approximately to [0, 1] range
        norm_features = [
            ...
            min(elbow_flare_score / 50.0, 1.0),        # Elbow Flare Score (0-50 norm)
            min(shoulder_asymmetry / 30.0, 1.0),       # Shoulder Asymmetry (0-30% norm)
            min(wrist_deviation / 50.0, 1.0),          # Wrist Deviation (0-50 norm)
        ]
    ...
    def train_synthetic(self):
        ...
        for _ in range(1000):
            ...
            if is_risk == 0:
                ...
                elbow = random.uniform(0.0, 0.15)
                shoulder = random.uniform(0.0, 0.15)
                wrist = random.uniform(0.0, 0.15)
            else:
                # Risk stats
                ...
                elbow = random.uniform(0.3, 1.0)
                shoulder = random.uniform(0.3, 1.0)
                wrist = random.uniform(0.3, 1.0)
                
            features = [knee_var, knee_meandev, asym, posture_deg, fatigue, elbow, shoulder, wrist]
